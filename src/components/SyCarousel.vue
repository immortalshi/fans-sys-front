<template>
    <div style="width: 100%; height: 100%; position: relative">
        ...
        <div class="img-item" v-for="(item, i) in imgList" :key="i">
            <img :src="require('@/assets/images' + item.url)" style="width: inherit; height: 100vh"
                :class="imgIndex === i ? 'show-img' : 'hide-img'">
            <div class="item-content">
                <div :class="imgIndex === i ? 'show-carousel-content' : 'carousel-content'">
                    {{ item.content }}
        ...
        <div class="point-container">
            <div v-for="(item, i) in imgList" :key="i" :class="imgIndex === i ? 'index-point' : 'point'"
                style="cursor: pointer" @mousemove="chooseIndex(i)" @mouseleave="leaveIndex()">
            </div>
        </div>
    </div>
</template>
...
setInterval(() => {
        if (!isOver.value) {
            let temp = imgIndex.value
            imgIndex.value = (temp + 1) % props.imgList.length
        }
    }, 8000)
